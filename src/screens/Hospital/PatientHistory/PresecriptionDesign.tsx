...
const PresecriptionDesign = ({route}: any) => {
  ...
  <p style="color: #000; font-size: 20px;" class="semibold">
  Address: <span style="font-size: 18px;; color: #000; margin-right:34px" class="regular">${`${item?.patientId?.addresses?.[0]?.address}-${item?.patientId?.addresses?.[0]?.city}`}</span>
  </p>
              <p style="font-size: 18px; color: #000;">
                Weight (Kg): ${`${item?.history?.weight}, BP:${item?.history?.bloodPressure?.systolicPressure}/${item?.history?.bloodPressure?.diastolicPressure} mmHg`}
              </p>
      ...
      RNFetchBlob.fs
        ...
        .then(() => {
          ...
          Alert.showSuccess(`PDF downloaded successfully as ${fileName}!`);
        })
        .catch(error => {
          Alert.showError('Failed to download PDF');
        });
    } catch (error: any) {
      Alert.showError('Failed to generate PDF');
    }
  };
  ...
  return (
    <Wrapper statusBarBagColor={'transparent'} statusBarStyle={'light-content'}>
      <ScrollView>
        <View
          style={{padding: RF(24), paddingBottom: RF(80), marginTop: RF(16)}}>
          <View
            style={{
              ...styles.row,
              paddingBottom: RF(30),
            ...
            <View>
              ...
              <Text color={'#000'} style={{width: RF(200)}}>
                {item?.doctorId?.qualifications}
              </Text>
              <Text color={'#000'}>PM&DC Reg:{item?.doctorId?.pmdcNumber}</Text>
            </View>
            ...
            <Text size={9} color={'#000'}>
              ...
              <Text size={11} color={'#000'} SFregular>
                ...
                <Text
                  ...
                  color={
                    '#000'
                  }>{`${item?.history?.bloodPressure?.systolicPressure}/${item?.history?.bloodPressure?.diastolicPressure} mmHg`}</Text>
              </Text>
            </Text>
          </Text>
          <View
          ...
          <View style={{borderBottomWidth: 1, paddingBottom: RF(16)}}>
            ...
            {item?.ePrescription?.test?.map((user: any, index: any) => (
          ...
          {item?.ePrescription?.medicines?.map((user: any, index: any) => (
            <View
              key={index}
              style={{
                ...styles?.row,
...
export default PresecriptionDesign;
