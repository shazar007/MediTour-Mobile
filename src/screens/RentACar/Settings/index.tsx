...
import {rentACar_UpdateProfile} from '@services';
import {useTheme} from '@react-navigation/native';
import {useDispatch, useSelector} from 'react-redux';
import {CustomLoader, Generic_Settings} from '@components';

const RentACar_Settings_B2B = () => {
  ...
  const updateProfile = () => {
    ...
    let params = {
      ...
      // location: B2B?.rentCar?.location,
      cnicOrPassportExpiry: settings?.cnicExpiry,
      accountHolderName: settings?.accHolderName,
      description: '',
      licenseImage: settings?.licenseImg,
    };
    rentACar_UpdateProfile(params)
      .then((res: any) => {
  ...
  return (
    <>
      <Generic_Settings
        onUpdate={onSubmit}
        email={B2B?.rentCar?.email}
        phone={B2B?.rentCar?.phoneNumber}
        address={B2B?.rentCar?.location?.address}
      />
...
export default RentACar_Settings_B2B;
